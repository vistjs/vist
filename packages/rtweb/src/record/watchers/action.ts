...
const isClickRecord = {
    ...
    handle: (record: eventWithTime)=>{
        return {type: RecordType.CLICK, x: (record.data as mouseInteractionData).x, y: (record.data as mouseInteractionData).y}
    },
}
...
const interestedRecords = [
    composeRule([isIncrementalSnapshot, isInterestedIncrementalSource, 
        [isClickRecord, isInputRecord]])
];
